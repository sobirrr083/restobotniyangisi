...
async def menu_actions(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if query.data == ADD_RESTAURANT:
        ...
        if not rated_restaurants:
            keyboard = [[InlineKeyboardButton("🔙 Orqaga", callback_data=CANCEL)]]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await query.edit_message_text("Hech qanday baholangan restoran topilmadi.", reply ${({

System: The message was cut off, but I understand you're addressing the `SyntaxError` in the `main.py` file and providing a corrected version. The error was in the `DELETE_RESTAURANT` section, and you've shared an updated `main.py` to fix it. Let me complete the response by providing the fully corrected code, ensuring the `DELETE_RESTAURANT` section is fixed, and verifying that all other functionality (admin panel, button-based restaurant list, etc.) is intact.

### Issue Recap
The `SyntaxError: '(' was never closed` on line 244 was caused by an incorrect line in the `DELETE_RESTAURANT` section:
...
The issues were:
1. Undefined variable `meals` (should be `keyboard`).
2. Incorrect syntax with `meals.append` inside the `edit_message_text` arguments.
3. Missing proper `reply_markup` parameter.

The corrected section should append the "Back" button to the `keyboard` list and pass the `InlineKeyboardMarkup` as `reply_markup`.

### Corrected Code
Below is the complete `main.py` with the fixed `DELETE_RESTAURANT` section. I've also ensured the code is clean, functional, and matches your requirements (admin panel, button-based restaurant list, detailed restaurant views).

<xaiArtifact artifact_id="64eb1021-a272-454f-a170-0fcf3fe6148a" artifact_version_id="f758dbf5-652a-481a-951d-ce94309b41e8" title="main.py" contentType="text/python">
import os
import logging
import json
from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
